...
// HP_IMPLEMENTATION_OPTION = 2 //Use a sequential version of the original hierarchical parallel implementation.
// HP_IMPLEMENTATION_OPTION = 3 //Use a hierarchical parallel version with scalar reductions. 

#define RED_IMPLEMENTATION_OPTION 2
// RED_IMPLEMENTATION_OPTION = 0 //Use original parallel_reduce() implementation.
// RED_IMPLEMENTATION_OPTION = 1 //Use a sequential version of the original parallel_reduce() implementation.
// RED_IMPLEMENTATION_OPTION = 2 //Use parallel_reduce() for each reduction variable
...
#if HP_IMPLEMENTATION_OPTION == 0
  //Use original hierarchical parallel implementation.
...
#elif HP_IMPLEMENTATION_OPTION == 1 
  //Use a single-level parallel_for()
  Kokkos::parallel_for("IntegrateStressForElems B", numNode,
                       KOKKOS_LAMBDA(const int k) {
    Index_t count = domain.nodeElemCount(k) ;
    Index_t *cornerList = domain.nodeElemCornerList(k) ;
    reduce_double3 f_tmp;
    f_tmp.x = 0.0;
    f_tmp.y = 0.0;
    f_tmp.z = 0.0;
    ...
    {
      Index_t elem = cornerList[i] ;
      f_tmp.x += fx_elem[elem] ;
      f_tmp.y += fy_elem[elem] ;
      f_tmp.z += fz_elem[elem] ;
    }
    domain.fx(k) = f_tmp.x ;
    domain.fy(k) = f_tmp.y ;
    domain.fz(k) = f_tmp.z ;
...
#elif HP_IMPLEMENTATION_OPTION == 2
  //Use a sequential version of the original hierarchical parallel implementation.
  Kokkos::parallel_for ( 1, KOKKOS_LAMBDA (int i)
  { 
    for ( int team_id = 0; team_id < (numNode+127)/128; team_id++ )
    {
      const Index_t gnode_begin = team_id*128;
      const Index_t gnode_end = (gnode_begin + 128<numNode)?gnode_begin + 128:numNode;
      for ( int gnode = gnode_begin; gnode < gnode_end; gnode++ )
      {
        Index_t count = domain.nodeElemCount(gnode) ;
        Index_t *cornerList = domain.nodeElemCornerList(gnode) ;
        reduce_double3 f_tmp;
...
#elif HP_IMPLEMENTATION_OPTION == 3
  //Use a hierarchical parallel version with scalar reductions. 
  Kokkos::parallel_for ("IntegrateStressForElems B-x",Kokkos::TeamPolicy<>((numNode+127)/128,team_size,2),
        KOKKOS_LAMBDA (const typename Kokkos::TeamPolicy<>::member_type& team)
     {
       const Index_t gnode_begin = team.league_rank()*128;
       const Index_t gnode_end = (gnode_begin + 128<numNode)?gnode_begin + 128:numNode;
       ...
       Kokkos::parallel_for(Kokkos::TeamThreadRange(team,gnode_begin,gnode_end),[&] (const Index_t& gnode) {
         ...
         Index_t *cornerList = domain.nodeElemCornerList(gnode) ;
         double f_tmp_x;
         Kokkos::parallel_reduce(Kokkos::ThreadVectorRange(team,count),[&](const Index_t& i,double& tmp_x) { //vectorized with ivdep
           Index_t elem = cornerList[i] ;
           tmp_x += fx_elem[elem] ;
         },f_tmp_x);
         Kokkos::single(Kokkos::PerThread(team), [&] () {
           domain.fx(gnode) += f_tmp_x ;
         });
       });
     });

  Kokkos::parallel_for ("IntegrateStressForElems B-y",Kokkos::TeamPolicy<>((numNode+127)/128,team_size,2),
        KOKKOS_LAMBDA (const typename Kokkos::TeamPolicy<>::member_type& team)
     {
       const Index_t gnode_begin = team.league_rank()*128;
       const Index_t gnode_end = (gnode_begin + 128<numNode)?gnode_begin + 128:numNode;
       ...
       Kokkos::parallel_for(Kokkos::TeamThreadRange(team,gnode_begin,gnode_end),[&] (const Index_t& gnode) {
         ...
         Index_t *cornerList = domain.nodeElemCornerList(gnode) ;
         double f_tmp_y;
         Kokkos::parallel_reduce(Kokkos::ThreadVectorRange(team,count),[&](const Index_t& i,double& tmp_y) { //vectorized with ivdep
           Index_t elem = cornerList[i] ;
           tmp_y += fy_elem[elem] ;
         },f_tmp_y);
         Kokkos::single(Kokkos::PerThread(team), [&] () {
           domain.fy(gnode) += f_tmp_y ;
         });
       });
     });

  Kokkos::parallel_for ("IntegrateStressForElems B-z",Kokkos::TeamPolicy<>((numNode+127)/128,team_size,2),
        KOKKOS_LAMBDA (const typename Kokkos::TeamPolicy<>::member_type& team)
     {
       const Index_t gnode_begin = team.league_rank()*128;
       const Index_t gnode_end = (gnode_begin + 128<numNode)?gnode_begin + 128:numNode;
       ...
       Kokkos::parallel_for(Kokkos::TeamThreadRange(team,gnode_begin,gnode_end),[&] (const Index_t& gnode) {
         ...
         Index_t *cornerList = domain.nodeElemCornerList(gnode) ;
         double f_tmp_z;
         Kokkos::parallel_reduce(Kokkos::ThreadVectorRange(team,count),[&](const Index_t& i,double& tmp_z) { //vectorized with ivdep
           Index_t elem = cornerList[i] ;
           tmp_z += fz_elem[elem] ;
         },f_tmp_z);
         Kokkos::single(Kokkos::PerThread(team), [&] () {
           domain.fy(gnode) += f_tmp_z ;
         });
       });
     });
#endif
...
static inline void CalcCourantConstraintForElems(Domain &domain, Index_t length,
...
#if RED_IMPLEMENTATION_OPTION == 0
  //Use original parallel_reduce() implementation.
  Kokkos::parallel_reduce(
      length, KOKKOS_LAMBDA(const int i, MinFinder &minf) {
                ...
                MinFinder tmp(dtf, i);
                //printf("original tmp = %2.1f\n", tmp.val );
                if (domain.vdov(indx) != Real_t(0.)) {
                  minf += tmp;
                  //printf("original in\n" );
                }
                //printf("original minf = %2.1f\n", minf.val );
              },
      result);

#elif RED_IMPLEMENTATION_OPTION == 1
  //Use a sequential version of the original parallel_reduce() implementation.
  //Work for OpenACC, but technically incorrect.
  Kokkos::View<MinFinder> result_d;
  Kokkos::parallel_for ( 1, KOKKOS_LAMBDA (int j) 
  {
    ...
    { 
      ...
      if (domain.vdov(indx) != Real_t(0.)) {
        //MinFinder_check(result_d, tmp);
        if(tmp.val < result_d().val) {
          result_d().val = tmp.val;
          result_d().i = tmp.i;
        }
...
#elif RED_IMPLEMENTATION_OPTION == 2
  //Use parallel_reduce() for each reduction variable
  Real_t result_val;
  int result_i;
  Kokkos::parallel_reduce(
      length, KOKKOS_LAMBDA(const int i, Real_t &result) {
                Index_t indx = domain.regElemlist(r,i);
                Real_t dtf = domain.ss(indx) * domain.ss(indx);
      ...
      Kokkos::Min<Real_t>(result_val));
  result.val = result_val;
  Kokkos::parallel_reduce(
      length, KOKKOS_LAMBDA(const int i, int &result) {
                Index_t indx = domain.regElemlist(r,i);
                Real_t dtf = domain.ss(indx) * domain.ss(indx);
      ...
      Kokkos::Max<int>(result_i));
  result.i = result_i;
#endif

  /*
  Kokkos::parallel_reduce(
      length, KOKKOS_LAMBDA(const int i, Real_t &minff) {
                minff = 100000000000000000000.0000;
                Index_t indx = domain.regElemlist(r,i);
                Real_t dtf = domain.ss(indx) * domain.ss(indx);
                ...
                //printf("new minf = %2.1f\n", minff );
                
              },
      Kokkos::Min<Real_t>(resultt));

  if (resultt == 100000000000000000000.0000)
  {
    ind = -1;
  }
  */
...
static inline void CalcHydroConstraintForElems(Domain &domain, Index_t length,
  ...
  Real_t result_val;
  int result_i;
  Kokkos::parallel_reduce(length, KOKKOS_LAMBDA(const int i, Real_t &result) {
                                    Index_t indx = domain.regElemlist(r,i);
                          ...
                          Kokkos::Min<Real_t>(result_val));
  result.val = result_val;
  Kokkos::parallel_reduce(length, KOKKOS_LAMBDA(const int i, int &result) {
                                    Index_t indx = domain.regElemlist(r,i);
