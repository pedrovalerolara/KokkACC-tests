...
static double elapsed_ui = 0.0, elapsed_yi = 0.0, elapsed_duidrj = 0.0,
              elapsed_deidrj = 0.0, elapsed_error=0.0, elapsed_forces=0.0;
...
int main(int argc, char* argv[]) {
...

    {
        ...
        printf("compute_forces = %f\n", elapsed_forces);
        printf("compute_error = %f\n", elapsed_error);
    }
    //void dumpTimers();
    //dumpTimers();
...
void compute() {
    ...
    elapsed = end - start;
    elapsed_forces += elapsed.count();

    start = system_clock::now();
    compute_error();
    end = system_clock::now();
    elapsed = end - start;
    elapsed_error += elapsed.count();
}
