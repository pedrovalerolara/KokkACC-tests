...
int main( int argc, char* argv[] )
{
  ...
  */
  ...
  Kokkos::parallel_reduce( "Sum reduction", team_policy( M, Kokkos::AUTO ), KOKKOS_LAMBDA ( const member_type &teamMember, float &update ) 
  {
    const int i = teamMember.league_rank();
    float tmp = 0.0;

    Kokkos::parallel_reduce( Kokkos::TeamThreadRange( teamMember, N ), [&] ( const int j, float &innerUpdate ) {
      innerUpdate += X[j + i * M];
    }, tmp );
...
*/
  ...
  /*
  Kokkos::parallel_reduce( "Prod reduction", team_policy( M, Kokkos::AUTO ), KOKKOS_LAMBDA ( const member_type &teamMember, float &update ) 
  {
    const int i = teamMember.league_rank();
    float tmp = 1.0;
